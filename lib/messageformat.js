...
MessageFormat.prototype._precompile = function(ast, data) {
  ...
  switch ( ast.type ) {
    ...
    case 'selectFormatPattern':
      ...
      addDefaultOtherFormIfNotExists(ast);
      var needOther = true;
...
function addDefaultOtherFormIfNotExists(ast) {
  // default other form is `other {#}`
  var defaultOtherForm = {"key":"other","val":{"type":"messageFormatPattern","statements":[{"type":"octothorpe"}]}};
  var hasOther = false;
  for (i = 0; i < ast.pluralForms.length; ++i) {
    var key = ast.pluralForms[i].key;
    if (key === 'other') hasOther = true;
  }
  if (!hasOther) ast.pluralForms.push(defaultOtherForm);
}
